...
const ChatbotModal = ({ book, contextItems = [] }: ChatbotModalProps) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "ai",
      content: "Hello! I'm Wiser. Ask me about any book, author, or insight.",
    },
  ]);
  ...
  const handleSendMessage = async (userMsg: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", content: output.answer, insights: output.insights },
      ]);
    } catch (e: any) {
      if (e.name === "CanceledError" || e.message === "canceled") {
        setMessages((prev) => [
          ...prev,
          { role: "ai", content: "Generation stopped by user." },
        ]);
      } else {
        setMessages((prev) => [
          ...prev,
    ...
    <>
      ...
      >
        ...
        <div className="fixed inset-0 flex items-center justify-center p-3 sm:p-6">
          <DialogPanel
            transition
            className="w-full max-w-5xl bg-gray-100 dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col h-[calc(100vh-120px)] sm:h-[calc(100vh-140px)] overflow-hidden border border-gray-300 dark:border-gray-700 transition-all duration-300 ease-out data-[closed]:scale-95 data-[closed]:translate-y-4 data-[closed]:opacity-0"
          >
            <div className="border-b border-gray-200 dark:border-gray-800 flex px-4 py-3 sm:py-4 justify-between items-center bg-white dark:bg-gray-800 sticky top-0 z-10 transition-colors duration-300">
              ...
              <button
                ...
                className="p-2 text-gray-500 dark:text-gray-300 cursor-pointer rounded-full transition-colors hover:bg-gray-100 dark:hover:bg-gray-700"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-6 px-4 py-5 sm:px-6 bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
              {messages.map((m, i) => (
                <div
                  key={i}
                  data-testid={
                    m.role === "user" ? "user-chat-bubble" : "ai-chat-bubble"
                  }
                  className={`flex gap-3 ${m.role === "user" ? "justify-end" : "justify-start"}`}
                >
                  <div
                    className={`flex flex-col gap-1 max-w-[85%] md:max-w-[75%] ${m.role === "user" ? "items-end" : "items-start"}`}
                  >
                    {/* BUBBLE CONTAINER */}
                    <div
                      className={`flex border border-gray-200 dark:border-gray-600 flex-col p-3 sm:p-4 text-sm leading-relaxed w-full transition-all duration-300 ${
                        m.role === "user"
                    ...
                    >
                        ...
                        <div
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter" && !e.shiftKey) {
                              ...
                              setEditingIndex(null);
                              if (editRef.current)
                                editRef.current.textContent = m.content || "";
                            }
                          }}
                            ...
                            components={{
                              ...
                              a: ({ href, children }) => (
                                <a
                                  ...
                                  className="text-blue-600 dark:text-blue-400 underline break-all hover:text-blue-800 dark:hover:text-blue-300 transition-colors"
                                >
                                  {children}
                                </a>
                              ),
                      ...
                      {/* AI Insights cards... */}
                      {m.insights &&
                        Object.entries(m.insights).map(
                          ([bookName, insightsList]) => (
                            <div key={bookName} className="space-y-3 mt-4">
                              <h2 className="text-base sm:text-lg font-bold border-b border-gray-200 dark:border-gray-700 pb-1">
                                &bull; {bookName}
                              </h2>
                    ...
                    {m.role === "ai" && m.content && (
                      <div className="pl-1 flex items-center gap-1 mt-0.5">
                        ...
                        >
                          ...
                          voiceMutation.variables ===
                            stripMarkdown(m.content ?? "").substring(0, 200) ? (
