...
class BookmarkService:
    """Service class to handle bookmark business logic, cleanly accepting DB and Cache dependencies."""
    
    def __init__(self, redis_client, repository):
        self.redis = redis_client
        self.repo = repository

    def toggle_bookmark_book(self, user_id: str, book_id: int) -> Dict[str, Any]:
        log_context = {
            "user_id": user_id,
            "book_id": book_id
        }
        ...
        try:
            result = self.repo.toggle_book(user_id, book_id)
            if result is None:
            ...
            self.redis.delete(f"bookmarks:books_data:{user_id}")
    ...
    def toggle_bookmark_insight(self, user_id: str, insight_id: int) -> Dict[str, Any]:
        log_context = {
            "user_id": user_id,
        ...
        try:
            result = self.repo.toggle_insight(user_id, insight_id)
            if result is None:
            ...
            self.redis.delete(f"bookmarks:insights_data:{user_id}")
            self.redis.delete(f"recommend:{user_id}")
            for key in self.redis.scan_iter(f"session_recommend:{user_id}:*"):
    ...
    def get_bookmarked_books_with_categories(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            books_data = self.repo.get_bookmarked_books(user_id)
    ...
    def get_bookmarked_insights_with_categories(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            insights_data = self.repo.get_bookmarked_insights(user_id)
...
def get_bookmark_service() -> BookmarkService:
    ...
    return BookmarkService(redis_client=redis_client, repository=BookmarkRepository)