...
def module_deps(monkeypatch, base_fake_deps):
    """
    Injects our global fake PostHog and Sentry instances into the voice handler.
    """
    posthog = base_fake_deps["posthog"]
    sentry = base_fake_deps["sentry"]

    monkeypatch.setattr(voice_controller, "posthog", posthog)
    monkeypatch.setattr(voice_controller.sentry_sdk, "capture_exception", sentry)
...
def test_generate_audio_success(mock_groq_client, module_deps):
    ...
    # 2. Act
    result = voice_controller.generate_audio_from_text(text=text_payload, voice="troy", user_id="user_123")
    ...
    assert posthog.capture.call_args_list[0].kwargs["event"] == "audio_generation_requested"
    assert posthog.capture.call_args_list[1].kwargs["event"] == "audio_generation_success"
...
def test_generate_audio_groq_failure(mock_groq_client, module_deps):
    ...
    mock_groq_client.audio.speech.create.side_effect = Exception("Insufficient Quota or 503 Service Unavailable")

    # 2. Act & Assert: FastAPI should catch the error and raise an HTTP 500
    with pytest.raises(HTTPException) as exc_info:
        voice_controller.generate_audio_from_text(text="Fail me.", voice="troy", user_id="user_123")
    ...
    assert posthog.capture.call_count == 2
    assert posthog.capture.call_args_list[1].kwargs["event"] == "audio_generation_failed"
    assert "Insufficient Quota" in posthog.capture.call_args_list[1].kwargs["properties"]["error"]